...
#     sig, sr = sf.read(file)
def extract_mfcc(sig, sr=16000, n_mfcc=16, n_fft=256, hop_length=128, n_mels = 40, delta_1 = False, delta_2 = False):

    mfcc = librosa.feature.mfcc(y=sig, sr=sr, n_mfcc=n_mfcc, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels)
...
def extract_lfcc(sig, sr=16000, num_ceps=20, order_deltas=2, low_freq=0, high_freq=8000):
    # sig, fs = sf.read(file)
...
def extract(data: pd.DataFrame, features):
    ...
    data = data.copy()
    data['features'] = data.apply(lambda x: extract_features(x, features), axis=1)
