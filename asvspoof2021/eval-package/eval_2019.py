...
# python asvspoof2021/eval-package/eval_2019.py docs/bio_tts_only_train_res/bio_trans_32concat_69_2019.txt patt_to_ASVspoof2019.LA.asv.eval.gi.trl.scores.txt tmp.txt
def calculate_tDCF_EER(cm_scores_file,
    ...
    if printout:
        ...
        dcf_breakdown = {
            attack_type: compute_tDCF(bona_cm,
    ...
    if printout:
        with open(output_file, "w") as f_res:
            ...
            f_res.write('\n==============\nmin-tDCF\t\t= {:8.9f}\n'.format(min_tDCF))
            f_res.write('\nBREAKDOWN tDCF SYSTEM\n')
            for attack_type in attack_types:
                _dcf = dcf_breakdown[attack_type] * 100
                
                _min_tDCF_index = np.argmin(_dcf)
                _min_tDCF = _dcf[_min_tDCF_index]
                
                f_res.write(
                    f'\tmin tCDF {attack_type}\t\t= {_min_tDCF:8.9f} % (Min tDCF for {attack_type}\n'
                )
